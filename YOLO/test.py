...
for person_name in os.listdir(dataset_path):
    ...
    if os.path.isdir(person_folder):  # Hanya proses folder
...
frame_skip = 1  # Proses hanya setiap 2 frame // Gajadi
...
while True:
    ...
    if frame_count % frame_skip == 0:
        # Lakukan deteksi YOLO hanya pada setiap frame yang diperlukan
        results = model(frame)  # Deteksi pada frame tertentu
        ...
        for result in results[0].boxes:
            x1, y1, x2, y2 = result.xyxy[0].cpu().numpy()  # Koordinat bounding box
            conf = result.conf[0].item()  # Confidence
            cls = result.cls[0].item()  # Kelas objek

            if conf > 0.5 and model.names[int(cls)] == "person":
                label = "person"  # Deteksi hanya manusia
                highest_confidence = max(highest_confidence, conf)

                person_id = len(current_ids) + 1  # Penentuan ID orang yang terdeteksi
                ...
                # Mengecek apakah face recognition perlu dijalankan
                if name == "Tidak dikenal" and \
                (person_id not in waktu_cek_face_recognition or (current_time - waktu_cek_face_recognition[person_id] > face_recognition_interval)): #Cek list Yang sudah biar ga double
                    face_frame = frame[int(y1):int(y2), int(x1):int(x2)]  # Crop wajah
                    ...
                    if face_encodings:
                        ...
                        if matches[best_match_index]:                                   #tar jadiin parameter index buat jika kesamaan lebih dari berapa persen bukan mencari ter mirip
                            name = known_face_names[best_match_index]
                            accuracy = (1 - face_distances[best_match_index]) * 100
                            id_tags[person_id] = name
                        else:
                            id_tags[person_id] = "Tidak dikenal"
                    waktu_cek_face_recognition[person_id] = current_time

                # Update waktu orang
                if name not in waktu_orang:
                    waktu_orang[name] = 0
                if name not in last_update_time_dict:
                    last_update_time_dict[name] = current_time

                waktu_orang[name] += current_time - last_update_time_dict[name]
                last_update_time_dict[name] = current_time
    ...
    # Tampilkan info
    cv2.putText(frame, f"GPU: {gpu_name}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.putText(frame, f"GPU Load: {gpu_load:.2f}%", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.putText(frame, f"Memory: {gpu_memory_used:.0f}/{gpu_memory_total:.0f} MB", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.putText(frame, f"YOLO Accuracy: {highest_confidence * 100:.2f}%", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    cv2.putText(frame, f"CPU Usage: {cpu_usage}%", (10, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
    cv2.putText(frame, f"RAM Usage: {memory_info.percent}%", (10, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
    cv2.putText(frame, f"FPS: {fps:.2f}", (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    # Tampilkan waktu kehadiran setiap orang
    y_offset = 250
    for nama, total_waktu in waktu_orang.items():
        cv2.putText(frame, f"{nama}'s Presence: {int(total_waktu)} detik",
                    (10, y_offset),
